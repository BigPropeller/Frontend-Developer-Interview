...
export class MessageEditor extends React.Component {
  ...
  updateInputValue(inputValue, caretPos, cb) {
    const caretPosition =
      caretPos || getCaretPosition(this.inputRef.current.textarea);
  ...
  render() {
    ...
    const mentionsOptionsFilter = items => {
      ...
      const itemsSortedByQuery = query
        ...
        : items;
      const notMentioned = item =>
        !this.state.mentions.find(m => m.id === item.id);
...
function replaceMentionQuery(message, mentionQuery, replacement) {
  ...
  fragments.push(
    message.substring(mentionQuery.index + mentionQuery.text.length + 1)
  );
...
function getTaggedMessage(message, mentions) {
  ...
  for (let i = 0; i < mentions.length; i++) {
    ...
    fragments.push(
      tagPlaceholders.open + mentions[i].name + tagPlaceholders.close
    );
